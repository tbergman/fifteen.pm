...
cd ../
# push gzipped assets 
aws $profile_arg s3 sync build/ $1 --exclude '*.mp4*' --exclude '*.webm*' --acl public-read --content-encoding gzip
# push non-gzipped assets
aws $profile_arg s3 sync build/ $1 --include '*.mp4*' --include '*.webm*' --acl public-read
# refresh cloudfront
