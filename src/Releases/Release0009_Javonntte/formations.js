...
function pickSubdivisionBucket(triangle, globalRadius) {
...
function filterGeometries(tile, neighborhoodCentroid, neighborhoodRadius, geometries, globalRadius) {
    const footprint = pickFootprint(tile, globalRadius);
    const height = pickHeight(tile, neighborhoodCentroid, neighborhoodRadius);
    return geometries[footprint][height];
}

function formatTile(tile, neighborhoodCentroid, neighborhoodRadius, geometries, globalRadius) {
    const allowedGeometries = filterGeometries(tile, neighborhoodCentroid, neighborhoodRadius, geometries, globalRadius);
    const subdivisionBucket = pickSubdivisionBucket(tile.triangle, globalRadius);
